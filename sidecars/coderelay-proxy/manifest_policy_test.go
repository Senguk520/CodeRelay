...
func TestBuildCodeRelayQuotaResponseAggregatesShortestWindowWithoutClamp(t *testing.T) {
	hourlyPresent := true
	weeklyPresent := true
	...
	response := buildCodeRelayQuotaResponse(&apiKeySpec{AccountIDs: accountIDs}, state, time.Now())
	...
	shortest := buildCodeRelayQuotaResponse(&apiKeySpec{AccountIDs: []string{"plus-with-hourly"}}, state, time.Now())
	...
	emptyScope := buildCodeRelayQuotaResponse(&apiKeySpec{}, state, time.Now())
...
func TestBuildCodeRelayQuotaResponseGroupsPlansAndPoolHealth(t *testing.T) {
	present := true
	...
	response := buildCodeRelayQuotaResponseWithAccounts(
	...
	applyCodeRelayQuotaAuthHealth(&response, &apiKeySpec{AccountIDs: []string{"plus-1", "plus-2", "team-1", "api-key-1"}}, state, []*coreauth.Auth{{
		ID:             "team-auth",
		Unavailable:    true,
...
func TestRelayServerCodeRelayQuotaRequiresKeyAndIsolatesAccountScopes(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestCodeRelaySelectorRestrictsAuthsToClientAPIKeyAccountScope(t *testing.T) {
...
func TestAuthPoolUnavailableErrorUsesCodeRelayLocale(t *testing.T) {
...
func TestCodeRelayLocaleIgnoresRequestAcceptLanguage(t *testing.T) {
	recorder := httptest.NewRecorder()
	...
	selector := &coderelaySelector{locale: "en-US"}
	_, err := selector.Pick(
	...
	if err == nil || !strings.Contains(err.Error(), "No available account") {
		t.Fatalf("request language must not override CodeRelay locale: %v", err)
	}
}
...
func TestCodeRelaySessionAffinitySeparatesClientAPIKeyScopes(t *testing.T) {
	highQuotaAccount := &accountSpec{
		ID:       "account-high",
		AuthID:   "account-high.json",
		PlanRank: intPtrForTest(500),
	...
	selector := &coderelaySessionAffinitySelector{
...
func TestCodeRelaySelectorPickSkipsBoundOAuthAtEitherQuotaReserve(t *testing.T) {
	tests := []struct {
		name            string
		hourlyRemaining int
		weeklyRemaining int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			selector := &coderelaySelector{manifest: &manifest{
				accountByAuthID: map[string]*accountSpec{
...
func TestCodeRelaySelectorPrefersAccountWithFewerImageJobs(t *testing.T) {
...
func TestCodeRelaySelectorPickIgnoresExplicitlyMissingQuotaWindow(t *testing.T) {
	hourlyThreshold := 10
	weeklyThreshold := 20
	snapshotUpdatedAt := time.Now().Unix()
	weeklyRemaining := 80
...
func TestCodeRelaySelectorPickFailsClosedForUnknownBoundOAuthQuota(t *testing.T) {
	hourlyThreshold := 10
	weeklyThreshold := 20
	snapshotUpdatedAt := time.Now().Unix()
	weeklyWindowPresent := false
...
func TestCodeRelaySelectorPickFailsClosedForInvalidQuotaSnapshotTimestamp(t *testing.T) {
	now := time.Now().Unix()
	tests := []struct {
		name      string
		timestamp *int64
...
func TestQuotaReserveSelectorFiltersCachedSessionAffinityAuth(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			selector := buildCoreAuthSelector(cfg, &coderelaySelector{manifest: m}, m, nil)
...
func TestManifestRegisteredModelsPreserveReasoningEffortThroughThinkingPipeline(t *testing.T) {
	...
	manager := buildCoreAuthManager(&config.Config{}, &coderelaySelector{}, nil, nil, nil, nil)
