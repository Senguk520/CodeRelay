...
func (s *coderelaySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*coreauth.Auth) (*coreauth.Auth, error) {
	_ = opts
...
// availability pass, which runs before coderelaySelector.Pick. It intentionally
...
func (s *coderelaySelector) ReportAuthSelectionFailure(ctx context.Context, provider, model string, candidates []*coreauth.Auth, err error) error {
	if s == nil {
		return err
	}
	if ctx == nil {
	...
	for _, auth := range auths {
		...
		stats.members = append(stats.members, poolMemberDiagnostic(auth, s.accountForAuth(auth), true, "available", "account passed CodeRelay policy checks; manager availability rejected it"))
	}
	// The manager may reject a candidate because of runtime cooldown state that
	// is not represented in CodeRelay's policy layer. Do not claim success for
...
func (s *coderelaySelector) filterAuthsForImageGeneration(auths []*coreauth.Auth) []*coreauth.Auth {
...
func (s *coderelaySelector) prioritizeAuthsForAPIKey(ctx context.Context, auths []*coreauth.Auth) []*coreauth.Auth {
...
func (s *coderelaySelector) filterAuthsForAPIKeyScope(ctx context.Context, auths []*coreauth.Auth) []*coreauth.Auth {
...
func (s *coderelaySelector) maxConcurrentImageRequests() int {
...
func (s *coderelaySelector) orderImageAuths(auths []*coreauth.Auth, start int) []*coreauth.Auth {
...
func (s *coderelaySelector) orderAuths(auths []*coreauth.Auth, start int) []*coreauth.Auth {
...
func (s *coderelaySelector) orderCustom(auths []*coreauth.Auth, start int) []*coreauth.Auth {
...
func weightedOrder(group []*coreauth.Auth, rules map[string]customRoutingRule, selector *coderelaySelector, start int) []*coreauth.Auth {
	if len(group) <= 1 {
		return group
	}
	total := 0
...
func (s *coderelaySelector) accountForAuth(auth *coreauth.Auth) *accountSpec {
	if s == nil {
		return nil
	}
...
func (s *coderelaySelector) emitAuthSelected(ctx context.Context, auth *coreauth.Auth, provider, model string, candidateAuths, availableAuths int) {
...
func authPoolUnavailableError(locale string, stats authPoolSelectionStats, detail string) *coreauth.Error {
	...
	var message string
	if chinese {
		message = fmt.Sprintf("账号池没有可用账号：候选 %d 个，不可用 %d 个，模型排除 %d 个，额度保留拦截 %d 个，生图策略拦截 %d 个。请前往 CodeRelay Tools 查看账号池诊断详情。",
		...
		message = fmt.Sprintf("No available account: candidates=%d, unavailable=%d, model_excluded=%d, quota_reserved=%d, image_policy_blocked=%d. Check account pool diagnostics in CodeRelay Tools.",
			stats.candidateAuths, stats.unavailableAuths, stats.modelExcludedAuths, stats.quotaReservedAuths, stats.imagePolicyBlockedAuths)
	}
...
func (s *coderelaySelector) emitAuthPoolUnavailable(
	ctx context.Context,
	provider string,
	model string,
...
func (s *coderelaySelector) rotatedIndex(account *accountSpec, start int) int {
...
func buildCoreAuthSelector(cfg *config.Config, selector coreauth.Selector, m *manifest, quota *quotaReserveStateStore) coreauth.Selector {
	...
	if cfg != nil && cfg.Routing.SessionAffinity {
		...
		selector = &coderelaySessionAffinitySelector{inner: selector}
...
type coderelaySessionAffinitySelector struct {
	inner coreauth.Selector
}

func (s *coderelaySessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*coreauth.Auth) (*coreauth.Auth, error) {
...
func (s *coderelaySessionAffinitySelector) ReportAuthSelectionFailure(ctx context.Context, provider, model string, candidates []*coreauth.Auth, err error) error {
	if reporter, ok := s.inner.(coreauth.AuthSelectionFailureReporter); ok {
		return reporter.ReportAuthSelectionFailure(ctx, provider, model, candidates, err)
	}
	return err
