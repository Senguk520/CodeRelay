...
func (s *relayServer) router() *gin.Engine {
	...
	router.GET(coderelayQuotaPath, s.handleCodeRelayQuota)
	router.POST("/v1/coderelay/auth/reset", s.handleResetAuthState)
	router.POST("/v1/coderelay/accounts/reset-scheduler", s.handleResetSchedulerState)
	...
	router.POST("/v1/coderelay/codebuddy/sync", s.handleCodebuddySyncModels)
...
type coderelayQuotaResponse struct {
	...
	Plans                    []coderelayQuotaPlanSummary `json:"plans,omitempty"`
	UpdatedAt                int64                     `json:"updatedAt,omitempty"`
	Stale                    bool                      `json:"stale"`
}

type coderelayQuotaPlanSummary struct {
...
func buildCodeRelayQuotaResponse(spec *apiKeySpec, state quotaPoolStateFile, now time.Time) coderelayQuotaResponse {
	return buildCodeRelayQuotaResponseWithAccounts(spec, state, now, nil)
}

func buildCodeRelayQuotaResponseWithAccounts(spec *apiKeySpec, state quotaPoolStateFile, now time.Time, accounts map[string]*accountSpec) coderelayQuotaResponse {
	...
	for _, accountID := range accountIDs {
		...
		if !exists {
			...
			result.Plans = append(result.Plans, coderelayQuotaPlanSummary{Plan: plan})
		}
...
func applyCodeRelayQuotaAuthHealth(result *coderelayQuotaResponse, spec *apiKeySpec, state quotaPoolStateFile, auths []*coreauth.Auth, now time.Time) {
...
func (s *relayServer) handleCodeRelayQuota(c *gin.Context) {
	spec, ok := s.requireAPIKey(c)
	if !ok {
		return
	}
	if strings.TrimSpace(s.quotaPoolStatePath) == "" {
		c.JSON(http.StatusOK, coderelayQuotaResponse{Version: 1, Scope: "api_key_account_pool"})
		return
	}
	...
	response := buildCodeRelayQuotaResponseWithAccounts(spec, state, time.Now(), s.manifest.accountByID)
